...
// Site público INO RUN 2026 — Phase 4 · Stylize
// Dados: gemini.md (preços via precoLoteAtual.ts). Logo: /logo.svg oficial.
...
const FAQ_ITEMS = [
  ...
  { q: 'Como são definidas as categorias?', a: 'Individual masculino e feminino, com premiação dividida por faixa etária — o que valoriza o corredor amador em cada idade. A categoria é calculada pela idade na data da prova (11/10/2026).' },
  ...
  { q: 'Posso me inscrever em grupo?', a: 'Sim! O sistema permite inscrição de múltiplos atletas em uma única sessão de compra.' },
];
...
export default function PublicSite({ onRegister, onAdmin, totalInscritos }: Props) {
  const { d, h, m, s } = useCountdown();
  const [openFaq, setOpenFaq] = useState<number | null>(null);

  const lote5kmAtual = precoLoteAtual('5km');
  const lote10kmAtual = precoLoteAtual('10km');
  ...
  return (
    <div className="bg-brand-bg text-brand-ink font-sans">
      ...
      <header className="relative overflow-hidden bg-white">
        {/* Formas orgânicas roxo+amarelo — assinatura da capa */}
        ...
        <div className="section-wrap relative py-14 md:py-20">
          ...
          {/* Logo textual hero — fonte display, enorme */}
          <div className="mt-4">
          ...
          <div className="mt-10 flex flex-wrap items-end gap-2.5">
            {([['DIAS', d], ['HORAS', h], ['MIN', m], ['SEG', s]] as [string, number][]).map(([lab, val], i) => (
              <div key={lab} className="flex items-end gap-2.5">
                ...
                {i < 3 && (
                  <span className="text-brand-purple-mid font-bold pb-7" style={{ fontSize: 38 }}>:</span>
                )}
              </div>
            ))}
          </div>
      ...
      <section id="provas" className="section-wrap section-pad">
        ...
        <div className="mt-8 grid gap-5 md:grid-cols-2">
          <ProvaCard
            id="5km" km="5" label="Prova 5 km" tag="Iniciante"
            ...
            preco={lote5kmAtual?.preco_centavos ?? 7900}
            onInscrever={onRegister}
          />
          <ProvaCard
            id="10km" km="10" label="Prova 10 km" tag="Performance"
            ...
            preco={lote10kmAtual?.preco_centavos ?? 9900}
            onInscrever={onRegister}
          />
        </div>
        ...
        <div className="mt-5 bg-brand-lilac rounded-2xl px-6 py-5">
          <div className="flex flex-wrap items-center gap-x-8 gap-y-2">
            ...
            <span className="text-[15px] text-brand-ink">
              <strong>Individual</strong> — masculino e feminino, com premiação por <strong>faixa etária</strong>:{' '}
        ...
        <div className="card p-6">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <h3 className="font-display font-extrabold italic uppercase text-[26px] mt-1.5 text-brand-ink">
                Lotes de inscrição
              </h3>
            </div>
            ...
            <div className="grid gap-3 md:grid-cols-3">
              {LOTES_5KM.map((l) => (
                <LoteCard key={l.id} nome={l.nome}
                  info={`${formataBRL(l.preco_centavos)} · até ${l.fecha_em.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}`}
                  ativo={precoLoteAtual('5km')?.id === l.id}
                />
              ))}
            </div>
          </div>
          ...
          <div className="mt-5">
            ...
            <div className="grid gap-3 md:grid-cols-3">
              {LOTES_10KM.map((l) => (
                <LoteCard key={l.id} nome={l.nome}
                  info={`${formataBRL(l.preco_centavos)} · até ${l.fecha_em.toLocaleDateString('pt-BR', { day: '2-digit', month: '2-digit' })}`}
                  ativo={precoLoteAtual('10km')?.id === l.id}
                />
              ))}
            </div>
          </div>
        ...
        <div className="section-wrap section-pad grid gap-8 md:grid-cols-2">
          ...
          <div className="bg-brand-lilac rounded-2xl min-h-[240px] relative overflow-hidden flex items-center justify-center">
            <svg viewBox="0 0 400 220" className="w-full h-full">
              ...
              <text x="350" y="35" fontSize="11" fill="#6E5E76" fontFamily="Inter">Chegada</text>
            </svg>
            <span className="absolute bottom-3 left-4 text-[12px] text-brand-muted">Mapa ilustrativo — traçado definitivo a confirmar</span>
          </div>
        </div>
      </section>
      ...
      <section id="kit" className="section-wrap section-pad">
        ...
        <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          {KIT_ITEMS.map((k, i) => (
            <KitItem key={k.item} numero={i + 1} item={k.item} det={k.det} />
          ))}
        </div>
      </section>
        ...
        <div className="mt-6 border-t border-brand-lilac-mid">
          {FAQ_ITEMS.map((f, i) => (
            <div key={i} className="border-b border-brand-lilac-mid">
              ...
              >
                {f.q}
                <span className="text-brand-purple text-2xl leading-none ml-4 flex-shrink-0">
                  {openFaq === i ? '−' : '+'}
                </span>
              </button>
              {openFaq === i && (
                <p className="text-brand-muted pb-5 text-[15px] leading-relaxed">{f.a}</p>
              )}
            </div>
          ))}
        </div>
      ...
      <section className="bg-gradient-brand border-t-[6px] border-brand-yellow">
        <div className="section-wrap py-16 text-center">
          ...
          <button id="cta-quero-correr" onClick={onRegister}
            className="mt-8 btn-accent text-xl px-10 py-4">
            Quero correr
          </button>
        </div>
      </section>
